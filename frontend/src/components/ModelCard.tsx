...
export function ModelCard({ model, onClick, isComparing, onCompareToggle }: ModelCardProps) {
  const typeInfo = modelTypes.find(t => t.value === model.type);
  const topBenchmark = model.benchmarks.reduce((a, b) => a.score > b.score ? a : b);
  ...
  return (
    ...
    >
      {model.featured && (
        <div className="absolute top-0 right-0 px-3 py-1 bg-gradient-primary text-primary-foreground text-xs font-medium rounded-bl-lg flex items-center gap-1">
      ...
      <div onClick={onClick} className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3 py-3 border-y border-border/50">
          ...
          <div className="text-center">
            ...
            <p className="text-sm font-medium font-mono">{model.contextWindow}</p>
          </div>
          <div className="text-center">
            <p className="text-xs text-muted-foreground">Best Score</p>
            <p className="text-sm font-medium font-mono text-primary">{topBenchmark.score}%</p>
          </div>
        </div>
