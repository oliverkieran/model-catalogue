...
import { 
  Code, Pencil, Image, FileText, BookOpen, Shield, 
  Video, FolderCode, Layers, Server, Settings, Lock,
  Globe, CheckCircle, Mic, Languages, Captions,
  Palette, Box, Brain, FlaskConical, Cpu, Star,
  Calendar, DollarSign, Zap, ArrowUpRight
} from 'lucide-react';
...
export function ModelDetail({ model, onClose }: ModelDetailProps) {
  ...
  const avgRating = model.opinions.reduce((sum, o) => sum + o.rating, 0) / model.opinions.length;
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-card border-border">
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                <DialogTitle className="text-3xl font-bold">{model.name}</DialogTitle>
              ...
              <div className="flex items-center gap-1 text-model-multimodal">
                {[...Array(5)].map((_, i) => (
                  <Star 
                    ...
                    className={cn(
                      "h-5 w-5",
                      i < Math.round(avgRating) ? "fill-current" : "fill-none opacity-30"
                    )} 
                  />
                ))}
              </div>
              <p className="text-sm text-muted-foreground">{model.opinions.length} reviews</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-4 gap-4 py-6 border-y border-border">
          ...
          <div className="text-center p-4 rounded-lg bg-secondary/30">
            ...
            <p className="font-mono font-semibold text-sm">{model.pricing.input}</p>
          </div>
        </div>

        <Tabs defaultValue="benchmarks" className="mt-6">
          <TabsList className="w-full justify-start bg-secondary/30 p-1">
            <TabsTrigger value="benchmarks" className="flex-1">Benchmarks</TabsTrigger>
            <TabsTrigger value="usecases" className="flex-1">Use Cases</TabsTrigger>
            <TabsTrigger value="opinions" className="flex-1">Opinions</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="opinions" className="mt-6 space-y-4">
            {model.opinions.map((opinion, idx) => (
              <div 
                key={idx}
                  ...
                  <div className="flex items-center gap-0.5 text-model-multimodal">
                    {[...Array(5)].map((_, i) => (
                      <Star 
                        ...
                        className={cn(
                          "h-4 w-4",
                          i < opinion.rating ? "fill-current" : "fill-none opacity-30"
                        )} 
                      />
                    ))}
                  </div>
                </div>
                <p className="text-muted-foreground italic">"{opinion.content}"</p>
              </div>
            ))}
          </TabsContent>
        </Tabs>
